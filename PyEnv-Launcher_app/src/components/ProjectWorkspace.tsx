...
export const ProjectWorkspace: React.FC<ProjectWorkspaceProps> = ({ currentPath, setCurrentPath, withLoading }) => {
  ...
  return (
    <Card
      ...
      icon={<Icon name="folderOpen" className="w-4 h-4 text-light-text-header dark:text-dark-text-header" />}
    >
      <div className="flex flex-col gap-3">
        ...
        <div className="flex gap-2">
          <Button
            onClick={() => setCurrentPath("/Users/developer/projects/sample-project")}
            size="sm"
            icon="folderOpen"
          >
            Select Directory
          </Button>
          <Button onClick={handleNewProject} size="sm" icon="plusCircle">
            New Project
          </Button>
          <Button onClick={() => alert("Opening in explorer...")} size="sm" icon="externalLink">
            Open in Explorer
          </Button>
        </div>
      </div>
