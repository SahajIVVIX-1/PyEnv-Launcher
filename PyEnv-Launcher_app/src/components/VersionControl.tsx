...
const CommitModal: React.FC<CommitModalProps> = ({ isOpen, onClose, onCommit, onSuggest }) => {
  ...
  return (
    <div className="modal-backdrop">
      <div className="bg-light-primary dark:bg-dark-primary p-6 rounded-lg shadow-xl w-full max-w-lg border border-light-border dark:border-dark-border">
        ...
        <form onSubmit={handleSubmit}>
          <textarea
            ...
            className="w-full h-32 bg-light-secondary dark:bg-dark-secondary border border-light-border dark:border-dark-border rounded-md p-2 mb-4 text-sm focus:ring-2 focus:ring-light-accent dark:focus:ring-dark-accent focus:outline-none transition"
            autoFocus
          />
...
export const VersionControl: React.FC<VersionControlProps> = ({ gitStatus, onCommit, onSuggestCommitMessage }) => {
  ...
  return (
    <Card
      ...
      icon={<Icon name="gitBranch" className="w-4 h-4 text-light-text-header dark:text-dark-text-header" />}
    >
      <div className="space-y-3">
        <div className="bg-light-secondary dark:bg-dark-secondary p-2 rounded-md text-sm">
          <p>
            ...
            {gitStatus.isDirty ? (
              <span className="text-dark-dirty ml-2 font-semibold">(dirty)</span>
            ) : (
              <span className="text-dark-success ml-2 font-semibold">(clean)</span>
            )}
          </p>
        </div>
        <div className="text-xs font-mono bg-light-secondary dark:bg-dark-secondary p-2 rounded-md max-h-24 overflow-y-auto">
          <p className="font-sans font-bold text-xs mb-1 text-light-text-secondary dark:text-dark-text-secondary">
            Recent Commits:
          </p>
          {gitStatus.log.map((l) => (
            <div key={l.hash} className="truncate hover:whitespace-normal">
              ...
              <span className="text-light-text-primary dark:text-dark-text-primary">{l.message}</span>
              <span className="text-light-text-secondary dark:text-dark-text-secondary ml-2">({l.date})</span>
            </div>
          ))}
        </div>
        <div className="flex justify-end gap-2">
          <Button size="sm" icon="cloudDownload">
            Pull
          </Button>
          <Button size="sm" icon="save" onClick={() => setCommitModalOpen(true)} disabled={!gitStatus.isDirty}>
            Commit
          </Button>
        </div>
      </div>
